...
app.use(express.json({limit:'4mb'})); app.use(express.urlencoded({extended:true})); app.use('/uploads',express.static(UP)); app.use(express.static(path.join(ROOT,'public')));
...
function collection(name,label){app.post('/api/'+name,(req,res)=>{const x={id:id(db[name]),createdAt:new Date().toISOString(),...req.body};db[name].push(x);if(name==='risks'){x.score=Number(x.probability)*Number(x.gravity);x.level=x.score>=16?'Critique':x.score>=9?'Élevé':x.score>=4?'Modéré':'Faible';}if(name==='inspections'&&x.observation){const a={id:id(db.actions),createdAt:new Date().toISOString(),title:'Action issue de l’inspection',description:x.observation,responsible:x.responsible||'',dueDate:x.dueDate||'',priority:x.riskLevel||'Moyenne',status:'Ouverte',source:'Inspection'};db.actions.push(a);db.notifications.unshift({id:id(db.notifications),date:new Date().toISOString(),type:'Action automatique',message:'Une action corrective a été créée depuis une inspection.'})}log('Utilisateur','Création '+label,String(x.id));res.json(x)});
...
save();app.listen(PORT,()=>console.log(`QHSE Automation PRO V5 : http://localhost:${PORT}`));
